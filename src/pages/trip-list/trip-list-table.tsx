...
const TripListTable = ({
  ...
  const handleRowClick = (trip: any) => {
    if (trip.isOrphaned) {
      ...
      navigate(`${paths.TRIPDETAILS}/${trip.tripId}/?vin=${trip.vin}`, {
        state: trip,
      });
    }
  };

  console.log('local date : ', getLocalDateFromUtc(localSearchQueries['endDate']));
              ...
              tripsInformation?.trips?.map((trip: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <StyledDataCell>{formatDateTimeForTripTable(startDate, startTime)}</StyledDataCell>
                    <StyledDataCell>{formatDateTimeForTripTable(endDate, endTime)}</StyledDataCell>
                    <StyledDataCell>
