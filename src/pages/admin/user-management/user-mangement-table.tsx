...
import OnboardingEmailImg from '../../../assets/icons/onboarding-email.png';
import { formatPhoneNumberForFleetPersonal, normalizeStatus } from '../../../utility/utilities';
...
const UserManagementTable = ({
  ...
  // Helper function to render role/org with additional count
  const renderRoleOrOrg = (mappings: any[], type: 'roles' | 'orgs', user: any) => {
    if (!mappings || mappings.length === 0) {
      return type === 'orgs' ? 'DronaAim' : 'NA';
    }

    const firstItem = mappings[0];
    const displayText = type === 'roles' ? roleLabels[firstItem.role] || firstItem.role : getOrgName(firstItem);

    const additionalCount = mappings.length - 1;
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'center' }}>
        ...
        {additionalCount > 0 && (
          <Chip
            ...
            onClick={(e) => handlePopoverOpen(e, mappings, type)}
          />
        )}
      </Box>
    );
  ...
  const columns = [
    ...
    { label: 'Name', key: 'firstAndLastName' },
    { label: 'Last Login', key: 'lastLoginAtDisplay' },
  ...
  return (
    <Box sx={{ maxWidth: '81vw', overflowX: 'auto' }}>
              ...
              usersInformation?.users?.map((user: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <StyledDataCell width="8%">
                      {formatPhoneNumberForFleetPersonal(user?.primaryPhone, user?.primaryPhoneCtryCd)}
                    </StyledDataCell>
                    <StyledDataCell>{renderRoleOrOrg(orgRoleMappings, 'orgs', user)}</StyledDataCell>
                    <StyledDataCell>{renderRoleOrOrg(orgRoleMappings, 'roles', user)}</StyledDataCell>
                    <StyledDataCell
                      sx={{
                        display: 'flex',
                        justifyContent: 'center',
                    ...
                    >
                      <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2 }}>
                        ...
                        <Tooltip title="Resend Onboarding Email" placement="bottom">
                          <IconButton onClick={() => handleOnboardingMail(user)}>
                            <img src={OnboardingEmailImg} alt="Onboarding Email" width={20} height={20} />
                          </IconButton>
                        </Tooltip>
                      </Box>
                    </StyledDataCell>
      ...
      {/* Popover for displaying additional roles/organizations */}
      <Popover
