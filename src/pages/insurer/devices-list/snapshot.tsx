...
import DeviceMountingNotification from './notification-popup';
...
const SnapshotImageDisplay: React.FC<SnapshotImageDisplayProps> = ({
  snapshots = [],
  mediaBaseUrl,
  lonestarId,
  vehicleId,
  ...
  const insurerId = useSelector((state: any) => state.auth.insurerId);
    ...
    <>
      <Box sx={{ display: 'flex', justifyContent: 'flex-start', px: 1, mb: 1 }}>
        <IconButton
          ...
          disabled={!snapshots?.some((snap) => snap.camera === 1 && snap?.urls?.[0]?.url)}
          sx={{
        ...
        >
          <Box component="img" src={bellIcon} alt="Notify" sx={{ width: 16, height: 16 }} />
          Notify
        </IconButton>
      </Box>
