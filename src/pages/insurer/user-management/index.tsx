...
import {
  ...
  getRoles,
  downloadUsers,
} from '../../../services/insurer/IUserMangtService';
...
const UserManagement: React.FC = () => {
  ...
  const fetchData = useCallback(
    async (page: number, params: any = {}) => {
      ...
      try {
        ...
        const { data } = await getInsurerUsers(currentUserId, page, itemsPerPage, requestBody);
  ...
  const fetchRoles = async (persona: string = 'admin') => {
    try {
      ...
      const filteredRoles = data?.roles?.filter((role: any) => ['insurer', 'insurerSuperUser'].includes(role));
  ...
  const handleUserManagementExport = async (fileType: 'xlsx' | 'csv' | 'pdf' = 'xlsx') => {
    ...
    try {
      ...
      const response = await downloadUsers(insurerId, currentUserId, currentUserOrg, fileType, 500, requestBody);
